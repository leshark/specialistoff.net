...
def generate4(os, listip, templatedir, num=0):
    j2_env=Environment(loader=FileSystemLoader(templatedir),
                     trim_blocks=True)
    for i, line in enumerate(listip):
        net = ipcalc.IPCalcV4(line)
        net.version='v4'
        template=j2_env.get_template(os + '.interfaces.template').render(i=(i+num), net=net)
        print(template)
...
def main():
    ...
    if args.ipv6:
        ...
        print(ipv4list)
        generate4(args.os, ipv4list, args.tmpldir, args.num)
